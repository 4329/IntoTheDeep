...
import org.firstinspires.ftc.teamcode.commands.TotalZeroCommand;
...
//  wait until navx is connected.       imuSubsystem = new ImuSubsystem(hardwareMap, telemetry);
        ...
        totalZeroCommandGroup = new TotalZeroCommand(elevatorSubsystem, clawSubsystem, telemetry);
        ...
        ElevatorVerticalCommand elevatorVerticalCommand = new ElevatorVerticalCommand(
            elevatorSubsystem,
            () -> operator.getLeftY(),
            telemetry
        );
       // robot has hardware locks ArmVerticalCommand armVerticalCommand = new ArmVerticalCommand(armSubsystem, () -> operator.getRightY(), telemetry);
...
// robot has hardware locks.        operator.getGamepadButton(GamepadKeys.Button.A).whenPressed(()-> armSubsystem.goToPosition(ArmPosition.IN));
...
//        operator.getGamepadButton(GamepadKeys.Button.A).whileHeld(()-> armSubsystem.armDown());
       ...
       // operator.getGamepadButton(GamepadKeys.Button.RIGHT_BUMPER).whenPressed(() -> armSubsystem.resetPidController());
        operator.getGamepadButton(GamepadKeys.Button.RIGHT_BUMPER).whenPressed(totalZeroCommandGroup);
